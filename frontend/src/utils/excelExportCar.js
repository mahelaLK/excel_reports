...
export const exportTableStyledCar = async (vesselName, gangDetailCars) => {
  try {
    const wb = XLSX.utils.book_new();
    const wsData = [];
    const merges = [];

    const plans = gangDetailCars.gangPlanDetails;
    const numGangs = plans.length;

    // per-gang column widths (3, or 4 if that gang has both VEH & BBLK)
    ...
    // the "sum" column block (used for Discharge/Apw/Overstow/etc totals)
    // and the Remarks(1) + per-gang Crane(numGangs) block in the shift
    // section both occupy the same width: 1 + numGangs
    ...
    // helper: sum a veh/bblk field across gangs IF all gangs share the same
    // veh-only / bblk-only type; if the vessel mixes both types (or a gang
    // has both), the total is left blank, mirroring the JSX's "both" branch.
    const uniformTotal = (getVehVal, getBblkVal) => {
      const anyBoth = plans.some(hasBothVehAndBblk);
      const anyVeh = plans.some(isVehOnly) || anyBoth;
      const anyBblk = plans.some(isBblkOnly) || anyBoth;
      if (anyBoth || (anyVeh && anyBblk)) return { value: "" };
      if (anyVeh) return { value: plans.reduce((s, p) => s + getVehVal(p), 0) };
      if (anyBblk) return { value: plans.reduce((s, p) => s + getBblkVal(p), 0) };
      return { value: "" };
    };
    ...
    // ---- Row 2: Berthside ----------------------------------------------
    wsData.push([
      mk(`Berthside: ${gangDetailCars.berthSide}`, {
    ...
    // ---- Row 6: Cont/BBLK/VEH ------------------------------------------
    {
      const row = [mk("Cont/BBLK/VEH", labelStyle(10))];
      plans.forEach((plan, i) => {
    ...
    // ---- Row 7: Discharge (shows NoOfVehicles / BBLK) ------------------
    {
      ...
      const total = uniformTotal(
        (p) => Number(p.details.NoOfVehicles) || 0,
        (p) => Number(p.details.BBLK) || 0
      );
      row.push(mk(total.value, totalStyle(10)));
    ...
    // ---- Apw Discharge (only if any gang has Apw data) -----------------
    if (plans.some((p) => p.details.ApwNoOfVehicles > 0 || p.details.ApwBBLK > 0)) {
      ...
      const total = uniformTotal(
        (p) => Number(p.details.ApwNoOfVehicles) || 0,
        (p) => Number(p.details.ApwBBLK) || 0
      );
      row.push(mk(total.value, totalStyle(10)));
      row.push(...emptyN(totalColWidth - 1));
      wsData.push(row);
    ...
    // ---- Overstow / Restow / Loads (conditional, plain values) ---------
    const plainMetricRow = (label, field) => {
      if (!plans.some((p) => Number(p.details[field]) > 0)) return;
    ...
    // ---- No of Lifts -----------------------------------------------
    {
      ...
      row.push(mk(total.value, { ...totalStyle(10), fill: { fgColor: { rgb: "D9D9D9" } } }));
      row.push(...emptyN(totalColWidth - 1));
      wsData.push(row);
      const r = wsData.length - 1;
      plans.forEach((plan, i) => {
    ...
    allShifts.forEach((shiftNo) => {
      ...
      baselineLiftTimes.forEach((lt, i) => {
        ...
        plans.forEach((plan, gangIdx) => {
          ...
          if (both) {
            row.push(
              mk(cell.bblkRaw != null ? cell.bblkRaw : "-", {
                ...valueStyle(9),
                fill: { fgColor: { rgb: "F2F2F2" } },
              })
            );
        ...
        // FM / Time Keeper / Drivers / Checklist / Traffic / Unlashing —
        // kept exactly as before: one field per row, only on the first
        // lift-time row of the shift.
        plans.forEach((plan) => {
          const shift = plan.shiftPlanDetails.find((s) => s.ShiftNumber === shiftNo);
          let staffInfo = "";
          ...
          row.push(
            mk(staffInfo, { font: { sz: 8 }, alignment: { horizontal: "left", vertical: "center" } })
          );
        });
    ...
    const calcTotalLiftsLeft = (plan) => {
      ...
      if (hasBothVehAndBblk(plan)) {
        ...
        const totalLiftsBBLK =
          (Number(d.Discharge) || 0) + (Number(d.Overstow) || 0) + (Number(d.Restow) || 0) +
          (Number(d.Loads) || 0) + (Number(d.BBLK) || 0) + (Number(d.ApwBBLK) || 0);
      ...
      const totalLifts =
        (Number(d.Discharge) || 0) + (Number(d.Overstow) || 0) + (Number(d.Restow) || 0) +
        (Number(d.Loads) || 0) + (Number(d[field]) || 0) + (Number(d[apwField]) || 0);
    ...
    const addSummaryRow = (label, calcFn, rowIndexForBblkSpan) => {
      const row = [mk(label, labelStyle(10))];
      const result = calcFn && plans.map(calcFn);
      const r = wsData.length; // this row's index (before push)

      plans.forEach((plan, i) => {
        const { veh, bblk } = result[i];
        const both = hasBothVehAndBblk(plan);
        row.push(mk(veh, valueStyle(10)));
        row.push(...emptyN(2));
        if (both) {
          row.push(mk(bblk === "" || bblk == null ? bblk ?? "" : bblk, valueStyle(10)));
        }
      });
    ...
    // The BBLK cell (for gangs with both VEH & BBLK) spans across all three
    // summary rows, matching the rowSpan={3} on "Total lifts left" in the JSX.
    plans.forEach((plan, i) => {
      if (hasBothVehAndBblk(plan)) {
