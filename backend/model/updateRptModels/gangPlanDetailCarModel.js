    ...
    try {
        ...
        const gangResult = await pool.request()
            ...
            .query(`SELECT 
                    ...
                    FROM  VPS_GangPlanHeader INNER JOIN VPS_Voyages ON VPS_GangPlanHeader.VoyageID = VPS_Voyages.VoyageID 
            ...
            for (const gangNumber of gangNumbers){
                ...
                .query(`
                    SELECT 
                        ...
                        VPS_GangPlanHeader.NoOfVehicles, 
                        VPS_GangPlanHeader.BBLK, 
                        VPS_GangPlanHeader.ApwNoOfVehicles, 
                        VPS_GangPlanHeader.ApwBBLK, 
                ...
                for (const row of detailResult.recordset){
                    ...
                    .query(`
                        SELECT VPS_ShiftPlanDetail.LiftTime
                        ...
                        GROUP BY VPS_ShiftPlanDetail.LiftTime
                    ...
                    for (const liftTimeRow of shiftDetails.recordset){
                        ...
                        .query(`
                            SELECT Target, Actual, BBLK, Remarks, Foreman, BayPlanner, Winchman, RDT
                        ...
                        winchman.push(...liftTimeDetails.recordset.flatMap(r=>r.Winchman?.trim()).filter(Boolean));
                ...
                gangPlanDetails.push({
                    ...
                    details: detailResult.recordset.map(r => ({
                        ...
                        BBLK: r.BBLK,
                        ApwNoOfVehicles: r.ApwNoOfVehicles,
                        ApwBBLK: r.ApwBBLK
                    }))[0],
                    shiftNumbers,
                    shiftPlanDetails
